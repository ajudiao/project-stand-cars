...
class ClientesController extends Controller
{
    ...
    public function store()
    {
        ...
        if ($this->clienteRepo->existsByEmailOrBI($cliente->email, $cliente->identidade)) {
            \App\Helpers\Helpers::setFlash('error', 'Já existe um cliente com este email ou BI.');
        ...
        try {
            ...
            if ($clienteId) {
                ...
                \App\Helpers\Helpers::setFlash('error', 'Erro ao criar cliente.');
                header('Location: /admin/clientes');
                exit;
            }
            ...
            \App\Helpers\Helpers::setFlash('error', 'Não foi possível criar o cliente. Verifique os dados e tente novamente.');
            header('Location: /admin/clientes');
            exit;
        }
    }
    ...
    public function show($id)
    {
        ...
        if (!$cliente) {
            \App\Helpers\Helpers::setFlash('error', 'Cliente não encontrado.');
            header('Location: /admin/clientes');
            exit;
        }
    ...
    public function buscar()
    {
        ...
        try {
            ...
            \App\Helpers\Helpers::setFlash('error', 'Erro ao buscar clientes. Tente novamente.');
            error_log($e->getMessage());
            header('Location: /admin/clientes');
            exit;
        }
    ...
    public function update($id)
    {
        ...
        $cliente->id = (int)$id;
         ...
         if ($this->clienteRepo->existsByEmailOrBIExcludingId($cliente->email, $cliente->identidade, $cliente->id)) {
            \App\Helpers\Helpers::setFlash('error', 'Já existe outro cliente com este email ou BI.');
            header('Location: /admin/clientes');
            exit;
        }

        if (!$this->clienteRepo->update($cliente)) {
            \App\Helpers\Helpers::setFlash('error', 'Erro ao atualizar cliente.');
            header('Location: /admin/clientes');
            exit;
        }

        \App\Helpers\Helpers::setFlash('success', 'Cliente atualizado com sucesso.');
        header('Location: /admin/clientes');
        exit;
    }
}
